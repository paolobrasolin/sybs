...
with open(file='Shri Mataji Talks.csv', mode='r', encoding='utf-8-sig') as csv_file:
    talks = [{key: value for key, value in row.items()}
             for row in csv.DictReader(csv_file)]

clean = {int(talk["post_id"]):{
    'title': talk['title'],
    'place': talk['place'],
    'city': talk['city'],
    'country': talk['Country'],
    'date': datetime.datetime.strptime(talk['Date'], '%Y-%m-%d').date(),
    'type': talk['type']}
         for talk in talks}

with open('amruta_meta.yml', 'w') as yml_file:
    yaml.dump(clean, yml_file, default_flow_style=False)
